...
[MemoryDiagnoser]
public class StreamProcessorIncrementalBenchmarks
{
    [Params(1_000_000)]
    ...
    public void Setup()
    {
        var markdown = GenerateMarkdown(FinalMarkdownSize);
        _chunks = SplitIntoChunks(markdown, ChunkSize);
    }
...
public void IterationSetup()
{
        ...
        int chunkSize)
    {
        ...
        for (var start = 0; start < markdown.Length; start += chunkSize)
        {
            var length =
                Math.Min(
                    chunkSize,
